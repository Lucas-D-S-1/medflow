...
def _validar_contrato(base: Path, nome: str) -> tuple[int, int]:
    contrato = json.loads((base / "contratos" / f"{nome}.json").read_text(encoding="utf-8"))
    tabelas = 0
    colunas = 0
...
def validar(base: Path) -> dict[str, int | str]:
    assert not list((base / "dados").rglob("*.parcial"))
    for antigo in ("raw", "processados", "curados", "referencias", "_backup_parquets_originais"):
        assert not (base / "dados" / antigo).exists(), antigo
    ...
    metadados_silver = json.loads(
        (base / "dados" / "silver" / "qualidade" / "METADADOS.json").read_text()
    )
    metadados_gold = json.loads(
        (base / "dados" / "gold" / "qualidade" / "METADADOS.json").read_text()
    )
    ...
    topo = json.loads(
        (base / "dados" / "gold" / "geografia" / "mapa_regiao_saude_sp.topojson").read_text()
    )
    assert topo["type"] == "Topology"
    assert len(topo["objects"]["regioes_saude"]["geometries"]) == 62

    inventario_pre = json.loads(
        (base / "contratos" / "INVENTARIO_PRE_MIGRACAO.json").read_text()
    )
    arquivos_atuais = [
        caminho
        for raiz in (base / "dados", base / "figuras")
